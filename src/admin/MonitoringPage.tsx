...
export default function AdminMonitoringPage() {
  ...
  const { data: predictions } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase
        .from('predictions')
        .select('match_id, player_id, pred_home, pred_away, is_absent')
        .in('match_id', matchIds)

      const map: Record<string, Record<string, { pred_home: number; pred_away: number; is_absent: boolean }>> = {}
      for (const p of data ?? []) {
        if (!map[p.match_id]) map[p.match_id] = {}
        map[p.match_id][p.player_id] = { pred_home: p.pred_home, pred_away: p.pred_away, is_absent: p.is_absent ?? false }
      }
      return map
    },
                    ...
                    return (
                      <td key={p.id} className="px-2 py-2.5 text-center">
                        {!pred ? (
                          ...
                          <span className="inline-flex items-center gap-0.5 text-xs font-bold text-text-dim">—</span>
                        ) : (
