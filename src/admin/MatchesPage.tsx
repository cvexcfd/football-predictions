    ...
    <div className="pb-20 max-w-3xl mx-auto">
      <div className="bg-gradient-to-r from-primary to-accent p-6 mb-6 text-white shadow-lg">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-2xl font-bold mt-1">Matches</p>
          </div>
          <Button variant="ghost" size="sm" className="text-white border border-white/30 hover:bg-white/20 rounded-xl" onClick={() => setShowForm(!showForm)}>
      ...
      <div className="flex gap-1 mb-4 overflow-x-auto">
        {(['upcoming', 'locked', 'finished', 'all'] as const).map(v => (
          <button
            key={v}
            className={`px-3 py-1.5 text-sm rounded-xl whitespace-nowrap transition-colors ${view === v ? 'bg-primary text-white shadow-sm' : 'bg-white text-text-muted border border-border/50 hover:bg-gray-50'}`}
            onClick={() => setView(v)}
          >
      ...
      {showForm && (
        <div className="bg-white rounded-xl shadow-sm border border-border/50 p-5 mb-4 space-y-3 animate-fade-in">
          <h2 className="font-semibold">New Match</h2>
          <select className="w-full px-3 py-2.5 border border-border rounded-xl text-sm bg-white outline-none focus:border-primary transition-colors" value={form.league_id} onChange={e => setForm(f => ({ ...f, league_id: e.target.value }))}>
            <option value="">Select league</option>
            {leagues?.map(l => <option key={l.id} value={l.id}>{l.name}</option>)}
          </select>
          {form.league_id && leagueTeams.length === 0 && (
            <div className="text-sm text-amber bg-amber/10 rounded-xl p-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <select className="px-3 py-2.5 border border-border rounded-xl text-sm bg-white outline-none focus:border-primary transition-colors" value={form.home_team_id} onChange={e => setForm(f => ({ ...f, home_team_id: e.target.value }))}>
              <option value="">Home team</option>
              {leagueTeams.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
            </select>
            <select className="px-3 py-2.5 border border-border rounded-xl text-sm bg-white outline-none focus:border-primary transition-colors" value={form.away_team_id} onChange={e => setForm(f => ({ ...f, away_team_id: e.target.value }))}>
          ...
          {showAddTeam && (
            <div className="border border-border/50 rounded-xl p-4 space-y-3 bg-gray-50/50 animate-fade-in">
          ...
          <div className="grid grid-cols-4 gap-2">
            <select className="px-3 py-2.5 border border-border rounded-xl text-sm bg-white outline-none focus:border-primary transition-colors" value={form.stage} onChange={e => setForm(f => ({ ...f, stage: e.target.value }))}>
      ...
      <div className="space-y-2">
        {matches?.map(m => (
          <div key={m.id} className="bg-white rounded-xl shadow-sm border border-border/50 p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className={`text-[10px] px-2.5 py-0.5 rounded-full font-medium ${
                ...
                m.status === 'locked' ? 'bg-amber-100 text-amber-700' :
            ...
            <div className="flex items-center gap-3 text-xs text-text-muted">
              <span className="bg-gray-100 px-2 py-0.5 rounded text-[10px]">E:{m.pts_exact} R:{m.pts_result} W:{m.pts_win}</span>
              {m.status === 'locked' && (
                <div className="flex items-center gap-1 ml-auto">
                  <input type="number" min="0" max="99" className="w-10 h-8 text-center border border-border rounded-lg text-sm outline-none focus:border-primary transition-colors" placeholder="H"
                    value={resultInputs[m.id]?.h ?? ''} onChange={e => setResultInputs(r => ({ ...r, [m.id]: { ...r[m.id], h: e.target.value } }))} />
                  <span className="text-text-muted">:</span>
                  <input type="number" min="0" max="99" className="w-10 h-8 text-center border border-border rounded-lg text-sm outline-none focus:border-primary transition-colors" placeholder="A"
